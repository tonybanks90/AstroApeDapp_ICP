...
persistent actor BondingCurveVault {
  ...
  public shared (msg) func set_ledger_id(pair : BasePair, ledger_id : Principal) : async Result.Result<(), Text> {
    if (not Principal.isController(msg.caller)) {
      return #err("Only controller can set ledger ID");
    };
    switch (pair) {
      case (#ckBTC) { ckbtcLedger := ledger_id };
      case (#ckETH) { ckethLedger := ledger_id };
    };
    return #ok(());
  };
  ...
  public shared (msg) func pullFunds(curveId : CurveId, user : Principal, amount : Nat) : async Result.Result<Nat, Text> {
    ...
    try {
      ...
      if (allowance.allowance < required) {
        return #err(
          "Insufficient allowance. Required: " # Nat.toText(required) #
          ", Available: " # Nat.toText(allowance.allowance)
        );
      };

      Debug.print(
        "Allowance check passed: " # Nat.toText(allowance.allowance) #
        " >= " # Nat.toText(required)
      );
    } catch (error) {
    ...
    switch (await ledgerActor.icrc2_transfer_from(transferArgs)) {
      case (#Ok(blockIndex)) {
        ...
        Debug.print(
          "Pulled " # Nat.toText(amount) # " from user " # Principal.toText(user) #
  ...
  public shared (msg) func payFunds(curveId : CurveId, user : Principal, amount : Nat) : async Result.Result<Nat, Text> {
    ...
    if (balance < required) {
      return #err(
        "Insufficient vault balance. Available: " # Nat.toText(balance) #
    ...
    switch (await ledgerActor.icrc1_transfer(transferArgs)) {
      case (#Ok(blockIndex)) {
        ...
        Debug.print(
          "Paid " # Nat.toText(amount) # " from curve " # Nat.toText(curveId) #
  ...
  public shared (msg) func updateSubaccountConfig(
    ...
    Debug.print(
      "Updated configuration for curve " # Nat.toText(curveId) #
  ...
  public func checkUserAllowance(user : Principal, basePair : BasePair, amount : Nat) : async Result.Result<{ allowance : Nat; sufficient : Bool; required : Nat }, Text> {
    let (ledger, fee) = switch (basePair) {
      case (#ckBTC) { (ckbtcLedger, ckbtcFee) };
      case (#ckETH) { (ckethLedger, ckethFee) };
    };
  ...
  public query func validateCurve(curveId : CurveId) : async Result.Result<{ exists : Bool; active : Bool; paused : Bool; basePair : ?BasePair; subaccount : ?[Nat8]; config : ?SubaccountConfig }, Text> {
  ...
  public query func getAllCurves() : async [{
    ...
    let buffer = Buffer.Buffer<{ curveId : CurveId; basePair : BasePair; active : Bool; paused : Bool; totalDeposited : Nat; totalWithdrawn : Nat; transactionCount : Nat }>(curveAccounts.size());
