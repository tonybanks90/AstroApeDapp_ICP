...
const Created = () => {
  // Find the token with id 1
  const token = tokenData.find((t) => t.id === 1);

  if (!token) return <p className="text-n-2 text-center">Token not found</p>;

  return (
    <div className="bg-n-8 border border-n-6 rounded-lg p-6 shadow-lg">
      {/* Table Header */}
      <div className="grid grid-cols-6 gap-4 pb-4 border-b border-n-6 text-n-2 text-sm font-medium uppercase">
        <div>TOKEN</div>
        <div>CREATED</div>
        <div>MKT CAP</div>
        <div>VOLUME</div>
      ...
      {/* Table Row */}
      <div className="grid grid-cols-6 gap-4 py-4 border-b border-n-6 items-center">
        {/* Token Info */}
