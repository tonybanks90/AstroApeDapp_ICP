...
export default function Guide({ onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-2 sm:px-4">
      <div className="relative bg-[#111] border border-[#333] rounded-2xl shadow-lg p-4 sm:p-6 max-w-md sm:max-w-2xl w-full text-white">
        {/* Close Button */}
        <button
          className="absolute top-3 right-3 text-gray-400 hover:text-white text-lg sm:text-xl"
          onClick={onClose}
        >
          <FaTimes />
        </button>
        ...
        {/* Header */}
        <h2 className="text-xl sm:text-2xl font-bold text-center mb-4 sm:mb-6 text-white/90">
          🚀 How to Use AstroApe
        </h2>
        ...
        <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
          {steps.map((step, index) => (
            <div
              key={index}
              className="flex items-start gap-3 bg-[#1a1a1a] p-3 sm:p-4 rounded-xl border border-[#222] hover:border-white/20 transition"
            >
              <div className="text-lg sm:text-xl text-color-1">{step.icon}</div>
              <div>
                <h4 className="text-sm sm:text-md font-semibold text-white/90">
                  {step.title}
                </h4>
        ...
        <div className="mt-4 sm:mt-6 text-center">
          <Button
            onClick={onClose}
            className="bg-color-1 text-black text-sm sm:text-base px-4 py-2 sm:px-6 sm:py-2 rounded-md hover:brightness-110"
          >
            Got it!
          </Button>
        </div>
